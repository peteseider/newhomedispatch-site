...
/* Conversion event tracking -- added 2026-07-27. Fires GA4 events for form
   submissions sitewide and for clicks on links into the three conversion
   pages (buyer-strategy-call, report-a-deal, subscribe-dispatch), so
   consultation/incentive-review/tour requests become measurable. */
(function () {
  function ready(fn) { if (document.readyState !== 'loading') { fn(); } else { document.addEventListener('DOMContentLoaded', fn); } }
  ready(function () {
    document.querySelectorAll('form').forEach(function (form) {
      form.addEventListener('submit', function () {
        var formName = form.getAttribute('name') || form.getAttribute('data-form-name') || form.id || 'unknown_form';
        if (typeof gtag === 'function') {
          gtag('event', 'form_submit', { form_name: formName, page_path: location.pathname });
        }
      });
    });
    document.querySelectorAll('a[href*="buyer-strategy-call"], a[href*="report-a-deal"], a[href*="subscribe-dispatch"]').forEach(function (a) {
      a.addEventListener('click', function () {
        if (typeof gtag === 'function') {
          gtag('event', 'cta_click', { cta_href: a.getAttribute('href'), cta_text: (a.textContent || '').trim().slice(0, 60), page_path: location.pathname });
        }
      });
    });
  });
