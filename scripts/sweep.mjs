...
async function detectChanges() {
  ...
  // Rotating 4-per-run slice keeps web_search use bounded; candidates come back
  // as NEW_BUILDER_OFFER for human verification (nothing publishes automatically).
  let watch = [];
  try {
  ...
  if (watch.length) {
    prompt += ' ADDITIONALLY, these builders are on our watchlist and not yet tracked. Check each one for any CURRENT advertised buyer incentive and include it in the same JSON array with status NEW_BUILDER_OFFER (advertised figure, community if stated, source URL); skip any with none advertised: ' + JSON.stringify(watch.map((w) => ({ builder: w.name, url: w.offersUrl })));
  }
